...
class BasePattern:
    ...
    # Optional name used to store match in results dict
    ...
    def match_seq(
            self,
            nodes: list[NL],
            results: Optional[_Results] = None) -> bool:
        """
    ...
    def generate_matches(
            self, nodes: list[NL]) -> Iterator[tuple[int, _Results]]:
        """
    ...
    def __init__(
        ...
        if content is not None:
            ...
            for i, item in enumerate(newcontent):
                ...
                # odd though *shrug*.
                if isinstance(
                        item, WildcardPattern):  # type: ignore[unreachable]
...
class WildcardPattern(BasePattern):
    ...
    def _recursive_matches(
            self, nodes, count) -> Iterator[tuple[int, _Results]]:
        ...
        if count < self.max:
            for alt in self.content:
                for c0, r0 in generate_matches(alt, nodes):
                    for c1, r1 in self._recursive_matches(
                            nodes[c0:], count + 1):
                        r = {}
                        r.update(r0)
                        r.update(r1)
