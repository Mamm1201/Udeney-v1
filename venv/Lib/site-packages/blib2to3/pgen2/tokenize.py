...
ContStr = group(
    _litprefix +
    "'" +
    _string_middle_single +
    group(
        "'",
        r"\\\r?\n"),
    _litprefix +
    '"' +
    _string_middle_double +
    group(
        '"',
        r"\\\r?\n"),
    group(
        _fstringlitprefix +
        "'") +
    _fstring_middle_single,
    group(
        _fstringlitprefix +
        '"') +
    _fstring_middle_double,
    group(
        _fstringlitprefix +
        "'") +
    _string_middle_single +
...
_strprefixes = _combinations("r", "R", "b", "B") | {
    "u", "U", "ur", "uR", "Ur", "UR"}
...
def tokenize(readline: Callable[[], str],
             tokeneater: TokenEater = printtoken) -> None:
    """
...
class Untokenizer:
    ...
    def compat(self, token: tuple[int, str],
               iterable: Iterable[TokenInfo]) -> None:
        startline = False
        indents = []
            ...
            if endmatch:
                ...
                if fstring_state.current() in (
                    ...
                    if is_fstring_start(token):
                        fstring_start, token = _split_fstring_start_and_middle(
                            token)
                        fstring_start_epos = (
                            spos[0], spos[1] + len(fstring_start))
                        yield (
                            FSTRING_START,
                            fstring_start,
                            spos,
                    ...
                    if token.endswith("{"):
                        ...
                        if token.endswith(('"""', "'''")):
                            fstring_middle, fstring_end = token[:-
                                                                3], token[-3:]
                    ...
                    if endmatch:  # all on one line
                        ...
                        if not is_fstring_start(token):
                            ...
                            if not token.endswith("{"):
                                fstring_middle, fstring_end = token[:-
                                                                    3], token[-3:]
                                fstring_middle_epos = fstring_end_spos = (
                                    lnum, end - 3)
                                yield (
                                    FSTRING_MIDDLE,
                                    fstring_middle,
                                ...
                                fstring_middle, lbrace = token[:-1], token[-1]
                                fstring_middle_epos = lbrace_spos = (
                                    lnum, end - 1)
                                yield (
                                    FSTRING_MIDDLE,
                                    fstring_middle,
