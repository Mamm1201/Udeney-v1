...
class Recorder:
    def __init__(
        ...
        self._points = {
            ilabel: stack_copy(
                self._start_point) for ilabel in ilabels}
...
class Parser:
    ...
    def __init__(
            self,
            grammar: Grammar,
            convert: Optional[Convert] = None) -> None:
        """Constructor.
    ...
    def addtoken(self, type: int, value: str, context: Context) -> bool:
        ...
        with self.proxy.release() as proxy:
            ...
            ilabel = cast(
                int, recorder.determine_route(
                    next_token_value, force=force))
            ...
            for i, newstate in arcs:
                ...
                if (0, state) in arcs:
                    ...
                    if not self.stack:
                        ...
                        raise ParseError(
                            "too much input", type, value, context)
                else:
    ...
    def classify(self, type: int, value: str, context: Context) -> list[int]:
        ...
        if type == token.NAME:
            ...
            if value in self.grammar.keywords:
                ...
                # it's just an expression, so we don't need to treat it
                # specially.)
