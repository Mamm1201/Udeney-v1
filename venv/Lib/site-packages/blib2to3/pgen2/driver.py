...
class Driver:
    ...
    def parse_stream_raw(self, stream: IO[str], debug: bool = False) -> NL:
        ...
        tokens = tokenize.generate_tokens(
            stream.readline, grammar=self.grammar)
    ...
    def _partially_consume_prefix(
            self, prefix: str, column: int) -> tuple[str, str]:
        lines: list[str] = []
        current_line = ""
        current_column = 0
        wait_for_nl = False
...
def load_packaged_grammar(
    ...
    if os.path.isfile(grammar_source):
        gp = _generate_pickle_name(
            grammar_source,
            cache_dir) if cache_dir else None
