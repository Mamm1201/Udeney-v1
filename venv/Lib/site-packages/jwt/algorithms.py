...
class Algorithm(ABC):
    ...
    @staticmethod
    @abstractmethod
    def to_jwk(key_obj, as_dict: Literal[True]
               ) -> JWKDict: ...  # pragma: no cover

    @overload
    @staticmethod
    @abstractmethod
    def to_jwk(key_obj, as_dict: Literal[False]
               = False) -> str: ...  # pragma: no cover
...
if has_crypto:
    ...
    class RSAAlgorithm(Algorithm):
        ...
        def prepare_key(self, key: AllowedRSAKeys |
                        str | bytes) -> AllowedRSAKeys:
        ...
        @staticmethod
        def to_jwk(key_obj: AllowedRSAKeys,
                   as_dict: bool = False) -> JWKDict | str:
    ...
    class ECAlgorithm(Algorithm):
        ...
        def prepare_key(self, key: AllowedECKeys |
                        str | bytes) -> AllowedECKeys:
            if isinstance(
                key,
                (EllipticCurvePrivateKey,
                 EllipticCurvePublicKey)):
            ...
            try:
                if key_bytes.startswith(b"ecdsa-sha2-"):
                    ...
                    crypto_key = load_pem_public_key(
                        key_bytes)  # type: ignore[assignment]
            except ValueError:
                crypto_key = load_pem_private_key(
                    key_bytes, password=None)  # type: ignore[assignment]
    ...
    class OKPAlgorithm(Algorithm):
        ...
        def prepare_key(self, key: AllowedOKPKeys |
                        str | bytes) -> AllowedOKPKeys:
        ...
        def verify(
            ...
            try:
                ...
                sig_bytes = sig.encode(
                    "utf-8") if isinstance(sig, str) else sig
        ...
        def to_jwk(
            ...
            if isinstance(key, (Ed25519PublicKey, Ed448PublicKey)):
                ...
                crv = "Ed25519" if isinstance(
                    key, Ed25519PublicKey) else "Ed448"
            ...
            if isinstance(key, (Ed25519PrivateKey, Ed448PrivateKey)):
                ...
                crv = "Ed25519" if isinstance(
                    key, Ed25519PrivateKey) else "Ed448"
                obj = {
                    "x": base64url_encode(force_bytes(x)).decode(),
