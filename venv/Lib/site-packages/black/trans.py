...
def handle_is_simple_look_up_prev(
        line: Line,
        index: int,
        disallowed: set[int]) -> bool:
    """
...
class CustomSplitMapMixin:
    ...
    _CUSTOM_SPLIT_MAP: ClassVar[dict[_Key,
                                     tuple[CustomSplit, ...]]] = defaultdict(tuple)
...
class StringMerger(StringTransformer, CustomSplitMapMixin):
    ...
    def _merge_one_string_group(self,
                                LL: list[Leaf],
                                string_idx: int,
                                is_valid_index: Callable[[int],
                                                         bool]) -> tuple[int,
                                                                         Leaf]:
        """
        ...
        def make_naked(string: str, string_prefix: str) -> str:
            ...
            if "f" in string_prefix:
                f_expressions = [
                    # +-1 to get rid of curly braces
                    string[span[0] + 1: span[1] - 1]
        ...
        # Temporary storage for the 'has_prefix' part of the CustomSplit
        # objects.
        prefix_tracker = []
        ...
        while is_valid_index(
                next_str_idx) and LL[next_str_idx].type == token.STRING:
            num_of_strings += 1
        ...
        # Fill the 'custom_splits' list with the appropriate CustomSplit
        # objects.
        ...
        if atom_node is not None:
            ...
            if non_string_idx - string_idx < len(atom_node.children):
                # We need to replace the old STRING leaves with the new string
                # leaf.
        ...
        for leaf in line.leaves[string_idx:]:
            ...
            if id(leaf) in line.comments:
                ...
                if contains_pragma_comment(line.comments[id(leaf)]):
                    return TErr(
                        "Cannot merge strings which have pragma comments.")
    ...
    def _transform_to_new_line(
        ...
        for idx in sorted(string_and_rpar_indices):
            ...
            if leaf.type == token.STRING:
                ...
                new_line.comments.setdefault(
                    id(string_leaf), []).extend(old_comments)
            else:
...
class BaseStringSplitter(StringTransformer):
    ...
    def do_match(self, line: Line) -> TMatchResult:
        ...
        string_indices = match_result.ok()
        assert len(string_indices) == 1, (f"{
            self.__class__.__name__} should only find one match at a time, found" f" {
            len(string_indices)}")
    ...
    def _validate(self, line: Line, string_idx: int) -> TResult[None]:
        ...
        if not string_leaf.parent or [
                L.type for L in string_leaf.parent.children] == [
                token.STRING, token.NEWLINE, ]:
            return TErr(
                f"This string ({
                    string_leaf.value}) appears to be pointless (i.e. has" " no parent).")
    ...
    def _get_max_string_length(self, line: Line, string_idx: int) -> int:
        ...
        if is_valid_index(string_idx - 1):
            ...
            if (
                ...
                # If the previous leaf is an empty LPAR placeholder, we should
                # skip it.
                p_idx -= 1

            P = LL[p_idx]
            if P.type in self.STRING_OPERATORS:
                # WMA4 a space and a string operator (e.g. `+ STRING` or `==
                # STRING`).
                offset += len(str(P)) + 1

            if P.type == token.COMMA:
                # WMA4 a space, a comma, and a closing bracket [e.g. `),
                # STRING`].
                offset += 3
        ...
        if is_valid_index(string_idx + 1):
            N = LL[string_idx + 1]
            if N.type == token.RPAR and N.value == "" and len(
                    LL) > string_idx + 2:
                # If the next leaf is an empty RPAR placeholder, we should skip
                # it.
                N = LL[string_idx + 2]
            ...
            if is_valid_index(string_idx + 2):
                ...
                if N.type == token.DOT and NN.type == token.NAME:
                    ...
                    # off of a string literal up to and including the LPAR
                    # character.
        ...
        max_string_length = count_chars_in_width(
            str(line), self.line_length - offset)
    ...
    def _prefer_paren_wrap_match(LL: list[Leaf]) -> Optional[int]:
        ...
        if (
            ...
            # And the string is surrounded by commas (or is the first/last
            # child)...
            ...
            if (
                ...
                # If it's an atom string, we need to check the parent atom's
                # siblings.
                parent = LL[0].parent
...
def iter_fexpr_spans(s: str) -> Iterator[tuple[int, int]]:
    ...
    while i < len(s):
        if s[i] == "{":
            # if we're in a string part of the f-string, ignore escaped curly
            # braces
        ...
        # note that backslashes are not legal in the expression portion of
        # f-strings
        if stack:
            delim = None
            if s[i: i + 3] in ("'''", '"""'):
...
class StringSplitter(BaseStringSplitter, CustomSplitMapMixin):
    ...
    def do_splitter_match(self, line: Line) -> TMatchResult:
        ...
        if (
            ...
            idx += 2
        # Else the first leaf MAY be a string operator symbol or the 'in'
        # keyword...
    ...
    def do_transform(
        ...
        ends_with_comma = (is_valid_index(string_idx + 1)
        ...
        max_break_width -= 1
        # Leading whitespace is not present in the string value (e.g.
        # Leaf.value).
        ...
        # line limit.
        use_custom_breakpoints = bool(custom_splits and all(
            csplit.break_idx <= max_break_width for csplit in custom_splits))
    ...
    def _iter_nameescape_slices(
            self, string: str) -> Iterator[tuple[Index, Index]]:
        """
        Yields:
        ...
        # False - the previous backslash was escaped *or* there was no
        # backslash
    ...
    def _get_break_idx(self, string: str, max_break_idx: int) -> Optional[int]:
        ...
        def passes_all_checks(i: Index) -> bool:
            ...
            """
            is_space = string[i] == " "
            is_split_safe = is_valid_index(
                i - 1) and string[i - 1] in SPLIT_SAFE_CHARS
        ...
        while is_valid_index(break_idx -
                             1) and not passes_all_checks(break_idx):
            break_idx -= 1
        ...
        if not passes_all_checks(break_idx):
            ...
            if not is_valid_index(
                    break_idx) or not passes_all_checks(break_idx):
                return None
    ...
    def _get_string_operator_leaves(
            self, leaves: Iterable[Leaf]) -> list[Leaf]:
        LL = list(leaves)
...
class StringParenWrapper(BaseStringSplitter, CustomSplitMapMixin):
    ...
    def _dict_or_lambda_match(LL: list[Leaf]) -> Optional[int]:
        ...
        Returns:
            ...
            None, otherwise.
        """
        # If this line is a part of a dictionary key assignment or lambda
        # expression...
        ...
        if syms.dictsetmaker in parent_types or syms.lambdef in parent_types:
            ...
            for i, leaf in enumerate(LL):
                # We MUST find a colon, it can either be dict's or lambda's
                # colon...
        ...
        # Otherwise, we use a lookup table to determine the next state.
            ...
            if (current_state, next_token) in self._goto:
                ...
                # error.
                    ...
                    raise RuntimeError(
                        f"{self.__class__.__name__} LOGIC ERROR!")
