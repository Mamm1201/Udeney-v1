...
class BracketTracker:
    ...
    def max_delimiter_priority(
            self, exclude: Iterable[LeafID] = ()) -> Priority:
...
def is_split_before_delimiter(leaf: Leaf,
                              previous: Optional[Leaf] = None) -> Priority:
    ...
    if leaf.value in {
        "if",
            "else"} and leaf.parent and leaf.parent.type == syms.test:
...
def get_leaves_inside_matching_brackets(leaves: Sequence[Leaf]) -> set[LeafID]:
    ...
    """
    try:
        # Start with the first opening bracket and ignore closing brackets
        # before.
        start_index = next(
