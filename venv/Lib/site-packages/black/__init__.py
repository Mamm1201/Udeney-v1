...
def validate_regex(
    ...
    try:
        ...
        raise click.BadParameter(
            f"Not a valid regular expression: {e}") from None
...
@click.option("-t",
              ...
              multiple=True,
              help=("Python versions that should be supported by Black's output. You should"
...
@click.option("--check",
              is_flag=True,
              help=("Don't write the files back, just return the status. Return code 0 means"
...
@click.option("--required-version",
              type=str,
              help=("Require a specific version of Black to be running. This is useful for"
...
@click.option("--include",
              ...
              callback=validate_regex,
              help=("A regular expression that matches files and directories that should be"
...
@click.option("-W",
              ...
              default=None,
              help=("When Black formats multiple files, it may use a process pool to speed up"
...
@click.version_option(
    version=__version__, message=(
        f"%(prog)s, %(version)s (compiled: {
...
def main(  # noqa: C901
    ...
    root, method = (
        find_project_root(
            src, stdin_filename) if code is None else (
            None, None))
    ctx.obj["root"] = root
...
def get_sources(
    *,
    ...
    exclude = re_compile_maybe_verbose(
    ...
    for s in src:
        if s == "-" and stdin_filename:
            ...
            if path_is_excluded(stdin_filename, force_exclude):
                report.path_ignored(
                    path, "--stdin-filename matches the --force-exclude regular expression", )
                continue
            is_stdin = True
        else:
            path = Path(s)
        ...
        if is_stdin or path.is_file():
            ...
            root_relative_path = best_effort_relative_path(
                path, root).as_posix()
...
def _format_str_once(
    ...
    line_generator = LineGenerator(
        mode=mode, features=context_manager_features)
...
def detect_target_versions(
    ...
    return {version for version in TargetVersion if features <=
            VERSION_TO_FEATURES[version]}
...
def get_future_imports(node: Node) -> set[str]:
    ...
    def get_imports_from_children(
            children: list[LN]) -> Generator[str, None, None]:
    ...
    for child in node.children:
        ...
        if isinstance(first_child, Leaf):
            ...
            if not isinstance(module_name,
                              Leaf) or module_name.value != "__future__":
                break
...
def _black_info() -> str:
    return (
        f"Black {__version__} on " f"Python ({
            platform.python_implementation()}) {
            platform.python_version()}")
