...
class LineGenerator(Visitor[Line]):
    ...
    def visit_simple_stmt(self, node: Node) -> Iterator[Line]:
        ...
        for child in node.children:
            if (prev_type is None or prev_type ==
                    token.SEMI) and is_arith_like(child):
                wrap_in_parentheses(node, child, visible=False)
    ...
    def visit_STRING(self, leaf: Leaf) -> Iterator[Line]:
        ...
        if is_docstring(leaf) and not re.search(r"\\\s*\n", leaf.value):
            ...
            if docstring:
                # Add some padding if the docstring starts / ends with a quote
                # mark.
                ...
                if docstring[-1] == quote_char:
                    docstring += " "
                if docstring[-1] == "\\":
                    backslash_count = len(docstring) - \
                        len(docstring.rstrip("\\"))
            ...
            # It's invalid to put closing single-character quotes on a new
            # line.
            if quote_len == 3:
...
def transform_line(
    ...
    # split the line. Default to line_str, if no power operator are present on
    # the line.
    ...
    if (
        ...
        # Only apply basic string preprocessing, since lines shouldn't be split
        # here.
    ...
    if (
        ...
        if hugged_opening_leaves and inner_body_leaves:
            ...
            if should_hug:
                ...
                # No need to re-calculate the body again later.
                body = inner_body
...
def _maybe_split_omitting_optional_parens(
    ...
    if (
        ...
        try:
            ...
            # For chained assignments we want to use the previous successful
            # split
...
def _prefer_split_rhs_oop_over_rhs(
    ...
    if not (len(rhs.head.leaves) >=
            2 and rhs.head.leaves[-2].type == token.EQUAL):
        return True
    ...
    # the body
    rhs_head_equal_count = [
        leaf.type for leaf in rhs.head.leaves].count(
        token.EQUAL)
    rhs_oop_head_equal_count = [
        leaf.type for leaf in rhs_oop.head.leaves].count(
        token.EQUAL)
...
def _ensure_trailing_comma(
    ...
    if any(leaf.type == token.COMMA and not is_part_of_annotation(leaf)
            for leaf in leaves):
        return False
