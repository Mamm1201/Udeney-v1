...
def find_project_root(
    ...
    src_parents = [list(path.parents) +
                   ([path] if path.is_dir() else []) for path in path_srcs]
...
def find_pyproject_toml(
    ...
    try:
        ...
        # We do not have access to the user-level config directory, so ignore
        # it.
...
def parse_pyproject_toml(path_config: str) -> dict[str, Any]:
    ...
    if "target_version" not in config:
        ...
        if inferred_target_version is not None:
            config["target_version"] = [v.name.lower()
                                        for v in inferred_target_version]
...
def parse_req_python_version(
        requires_python: str) -> Optional[list[TargetVersion]]:
...
def parse_req_python_specifier(
        requires_python: str) -> Optional[list[TargetVersion]]:
...
def strip_specifier_set(specifier_set: SpecifierSet) -> SpecifierSet:
    ...
    for s in specifier_set:
        if "*" in str(s):
            ...
            stripped = Specifier(
                f"{s.operator}{version.major}.{version.minor}")
...
def best_effort_relative_path(path: Path, root: Path) -> Path:
    ...
    try:
        ...
        pass
    root_parent = next(
        (p for p in path.parents if _cached_resolve(p) == root), None)
...
def _path_is_ignored(
    ...
    # ensure that gitignore_dict is ordered from least specific to most
    # specific.
...
def gen_python_files(
    ...
    *,
    ...
    for child in paths:
        ...
        # Then ignore with `--exclude` `--extend-exclude` and `--force-exclude`
        # options.
        ...
        if child.is_dir():
            ...
            include_match = include.search(
                root_relative_path) if include else True
            if include_match:
                yield child
...
def wrap_stream_for_windows(
    ...
    try:
        ...
        # Set `strip=False` to avoid needing to modify
