...
def whitespace(leaf: Leaf, *, complex_subscript: bool, mode: Mode) -> str:  # noqa: C901
    ...
    assert p is not None, f"INTERNAL ERROR: hand-made leaf without parent: {
        leaf!r}"
    ...
    if p.type in {syms.parameters, syms.arglist}:
        ...
        if t == token.EQUAL:
            ...
            # A bit hacky: if the equal sign has whitespace, it means we
            # previously found it's a typed argument.  So, we're using that,
            # too.
            return prev.prefix
...
def prev_siblings_are(node: Optional[LN],
                      tokens: list[Optional[NodeType]]) -> bool:
...
def is_docstring(node: NL) -> bool:
    ...
    if prev_siblings_are(
        node.parent, [
            syms.parameters, token.COLON, syms.simple_stmt]):
...
def is_type_ignore_comment(leaf: Leaf) -> bool:
    ...
    return t in {
        token.COMMENT,
        STANDALONE_COMMENT} and is_type_ignore_comment_string(v)
