...
def adjusted_lines(
    ...
    """
    lines_mappings = _calculate_lines_mappings(
    ...
    for start, end in sorted(lines):
        ...
        if start_mapping_index >= len(
                lines_mappings) or end_mapping_index >= len(lines_mappings):
        ...
        if start_mapping.is_changed_block:
            # When the line falls into a changed block, expands to the whole
            # block.
            new_start = start_mapping.modified_start
        else:
            new_start = (
                start -
                start_mapping.original_start +
                start_mapping.modified_start)
