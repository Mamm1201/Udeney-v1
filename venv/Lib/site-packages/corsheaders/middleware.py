...
class CorsMiddleware:
    ...
    def check_preflight(self, request: HttpRequest) -> HttpResponseBase | None:
        ...
        """
        request._cors_enabled = self.is_enabled(
            request)  # type: ignore [attr-defined]
        if (
    ...
    def add_response_headers(
        ...
        if request.method == "OPTIONS":
            ...
            if conf.CORS_PREFLIGHT_MAX_AGE:
                response[ACCESS_CONTROL_MAX_AGE] = str(
                    conf.CORS_PREFLIGHT_MAX_AGE)

        if (conf.CORS_ALLOW_PRIVATE_NETWORK and request.headers.get(
    ...
    def origin_found_in_white_lists(
            self, origin: str, url: SplitResult) -> bool:
        return (
