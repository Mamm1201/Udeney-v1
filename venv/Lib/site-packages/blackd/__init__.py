...
def parse_mode(headers: MultiMapping[str]) -> black.Mode:
    ...
    skip_magic_trailing_comma = bool(
        headers.get(SKIP_MAGIC_TRAILING_COMMA, False))
    ...
    enable_unstable_features = headers.get(
        ENABLE_UNSTABLE_FEATURE, "").split(",")
...
def parse_python_variant_header(
        value: str) -> tuple[bool, set[black.TargetVersion]]:
    if value == "pyi":
        return True, set()
    else:
        versions = set()
        for version in value.split(","):
            ...
            try:
                ...
                raise InvalidVariantHeader(
                    "expected e.g. '3.7', 'py3.5'") from None
