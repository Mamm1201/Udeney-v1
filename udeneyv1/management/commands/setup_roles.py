...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("Iniciando configuración del sistema de roles...")
        )
        ...
        self.stdout.write(
            self.style.SUCCESS("[SUCCESS] Sistema de roles configurado exitosamente")
        )
    ...
    def create_groups_and_permissions(self):
        ...
        from udeneyv1.models import Articulos, Calificaciones, Transacciones, Usuarios
        ...
        for group_name, permission_codenames in groups_config.items():
            ...
            for perm_codename in permission_codenames:
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"    [WARNING] Permiso no encontrado: {perm_codename}"
                        )
                    )

            self.stdout.write(
                f"    [OK] {len(permission_codenames)} permisos asignados a {group_name}"
            )
    ...
    def migrate_existing_users(self):
        ...
        role_mapping = {"vendedor": "Vendedor", "comprador": "Comprador"}
        ...
        for usuario_rol in usuario_roles:
            try:
                ...
                if new_group_name:
                    ...
                    self.stdout.write(
                        f"  [OK] Usuario {django_user.username} -> Grupo {new_group_name}"
                    )
                    migrated_count += 1
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f"  [WARNING] Rol no mapeado: {old_role} para usuario {django_user.username}"
                        )
                    )
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"  [WARNING] Usuario Django no encontrado para ID: {usuario_rol.id_usuario.id_usuario}"
                    )
                )
    ...
    def create_default_superadmin(self):
        ...
        try:
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "[SUCCESS] Usuario superadmin creado exitosamente"
                    )
                )
                self.stdout.write(
                    self.style.WARNING(
                        "[WARNING] IMPORTANTE: Cambie la contraseña por defecto (admin123456)"
                    )
                )
            else:
