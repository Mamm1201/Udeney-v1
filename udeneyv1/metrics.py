...
class MetricsCollector:
    ...
    def get_metrics(self, metric_name: str = None, since: datetime = None) -> Dict:
        ...
        with self._lock:
            ...
            return {
                name: list(data) for name, data in matching_metrics.items()
            }
...
class ApplicationMetrics:
    ...
    def track_authentication(email: str, success: bool):
        ...
        if not success:
            metrics.increment('auth_failures_total', tags={'email_domain': email.split('@')[-1]})
    ...
    def get_system_health() -> Dict:
        ...
        try:
            ...
            health_status['checks']['cache'] = {
                ...
                'message': 'Cache operations working' if cache_result == 'ok' else 'Cache error'
            }
...
def export_metrics_to_prometheus() -> str:
    ...
    for metric_name, data in summary.items():
        if data['type'] == 'counter':
            ...
            prometheus_output.append(f"{base_name}_duration_seconds_sum {data['total']}")
            prometheus_output.append(f"{base_name}_duration_seconds_count {data['count']}")
