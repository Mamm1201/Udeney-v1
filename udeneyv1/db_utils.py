...
class QueryOptimizer:
    ...
    def get_articulos_with_relations():
        ...
        return Articulos.objects.select_related(
            'id_categoria', 
            'id_usuario'
        ).prefetch_related(
            'calificaciones_set',
            'articulodetalletransaccion_set'
    ...
    def get_transacciones_with_details():
        ...
        return Transacciones.objects.select_related(
            'usuario'
        ).prefetch_related(
    ...
    def get_usuario_complete_profile(usuario_id):
        ...
        return Usuarios.objects.select_related().prefetch_related(
...
def analyze_query_performance():
    ...
    try:
        ...
        logger.info(f"Artículos optimizados: {len(articulos)} items, "
        ...
        logger.info(f"Transacciones optimizadas: {len(transacciones)} items, "
...
def get_slow_queries(min_time=0.1):
    ...
    slow_queries = [
        ...
        if float(query.get('time', 0)) > min_time
    ]
