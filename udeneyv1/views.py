...
# Logging utilities
from .logging_utils import SecurityLogger, AuditLogger, log_api_call
...
class UsuariosViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated]  # Solo usuarios autenticados pueden ver otros usuarios
...
class ArticulosViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Articulos.objects.filter(disponible=True).select_related('id_categoria', 'id_usuario')
    ...
    def perform_create(self, serializer):
        ...
        AuditLogger.log_model_change(
            self.request.user, 'Articulo', articulo.id_articulo, 'CREATE',
            {'titulo': articulo.titulo_articulo, 'precio': str(articulo.precio_articulo)}
        )
        # Business metrics
        ApplicationMetrics.track_business_event('articulo_created', self.request.user.id_usuario)
    ...
    def perform_update(self, serializer):
        ...
        # Audit log
        AuditLogger.log_model_change(
            self.request.user, 'Articulo', instance.id_articulo, 'UPDATE',
    ...
    def perform_destroy(self, instance):
        # Audit log
        AuditLogger.log_model_change(
            self.request.user, 'Articulo', instance.id_articulo, 'DELETE',
    ...
    def mis_articulos(self, request):
        ...
        articulos = Articulos.objects.filter(id_usuario=id_usuario).select_related('id_categoria', 'id_usuario')
...
class TodosArticulosViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = ArticulosSerializer
    permission_classes = [AdminPermissions]  # Solo administradores pueden ver todos los artículos
...
class CategoriasViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated]  # Solo usuarios autenticados pueden ver categorías
...
class DetalleTransaccionViewSet(viewsets.ModelViewSet):
    queryset = DetalleTransaccion.objects.select_related('id_transaccion', 'id_transaccion__usuario')
...
class TransaccionesViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [TransaccionPermissions]  # Permisos específicos para transacciones
    
    def get_queryset(self):
        # Los usuarios solo pueden ver sus propias transacciones
        if hasattr(self.request.user, 'id_usuario'):
            return Transacciones.objects.filter(usuario=self.request.user).select_related('usuario')
    ...
    def perform_create(self, serializer):
        ...
        # Business metrics
        ApplicationMetrics.track_business_event('transaccion_created', self.request.user.id_usuario)
...
class MisTransaccionesView(APIView):
    ...
    def get(self, request, id_usuario):
        ...
        if not hasattr(request.user, 'id_usuario') or request.user.id_usuario != int(id_usuario):
            return Response({"error": "No tienes permiso para ver estas transacciones"}, 
                          status=status.HTTP_403_FORBIDDEN)
        
        transacciones = Transacciones.objects.filter(usuario_id=id_usuario).select_related('usuario')
...
class PagosViewSet(viewsets.ModelViewSet):
    queryset = Pagos.objects.select_related('id_detalle_transaccion', 'id_detalle_transaccion__id_transaccion')
