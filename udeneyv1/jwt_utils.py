...
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def get_token(cls, user):
        ...
        main_permissions = [
            perm for perm in role_info['permissions'] 
            if 'udeneyv1.' in perm  # Solo permisos de nuestra app
        ]
        token['permissions'] = main_permissions[:10]  # Limitar a 10 permisos principales
...
def get_user_permissions_summary(user):
    ...
    permissions_summary = {
        'can_view_transactions': (
            ...
            bool(set(groups) & {'Vendedor', 'Comprador', 'Admin_Negocio', 'Monitor'})
        ),
        'can_create_articles': (
            user.is_superuser or 
            'Vendedor' in groups
        ...
        'can_manage_users': (
            user.is_superuser or 
            'Admin_Negocio' in groups
        ),
        'can_view_reports': (
            user.is_superuser or 
            bool(set(groups) & {'Admin_Negocio', 'Monitor'})
        ),
