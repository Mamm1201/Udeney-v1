...
from udeneyv1.permissions_new import (get_user_groups, get_user_permissions_summary,
                                      user_has_group)
...
class RolesMigrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.add_articulos_perm = Permission.objects.get(
            codename="add_articulos", content_type=articulos_ct
        )
        self.view_articulos_perm = Permission.objects.get(
            codename="view_articulos", content_type=articulos_ct
        )
        self.add_transacciones_perm = Permission.objects.get(
            codename="add_transacciones", content_type=transacciones_ct
        )

        # Asignar permisos a grupos
        self.vendedor_group.permissions.add(
            self.add_articulos_perm, self.view_articulos_perm
        )
        self.comprador_group.permissions.add(
            self.add_transacciones_perm, self.view_articulos_perm
        )
...
class PermissionsAPITestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.vendedor_user = User.objects.create_user(
            username="vendedor@test.com", password="testpass123"
        )
        self.vendedor_user.groups.add(self.vendedor_group)

        # Crear usuario admin
        self.admin_user = User.objects.create_user(
            username="admin@test.com", password="testpass123", is_staff=True
        )
        self.admin_user.groups.add(self.admin_group)
...
class LoginWithRolesTestCase(TestCase):
    ...
    def test_login_returns_role_info(self):
        ...
        login_data = {"email": "test@eduney.com", "password": "testpass123"}
...
class BackwardsCompatibilityTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="legacy_user", password="testpass123"
        )
        ...
        UsuarioRol.objects.create(id_usuario=self.eduney_user, id_rol="vendedor")
