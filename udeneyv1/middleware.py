...
from .models import Usuarios, UsuarioRol
from .logging_utils import SecurityLogger, PerformanceLogger, middleware_logger
    ...
    def process_request(self, request):
        ...
        if hasattr(request, 'user') and hasattr(request.user, 'id_usuario'):
            try:
                ...
                roles = UsuarioRol.objects.filter(id_usuario=usuario).values_list('id_rol', flat=True)
                request.user_roles = list(roles)
                
                middleware_logger.info(f"Usuario autenticado: {usuario.email_usuario} con roles: {request.user_roles}")
                
            except Usuarios.DoesNotExist:
                middleware_logger.warning(f"Usuario JWT no encontrado en BD: {request.user}")
            ...
            if request.path.startswith(route_pattern):
                ...
                if required_roles:
                    ...
                    if not any(role in request.user_roles for role in required_roles):
                        SecurityLogger.log_permission_denied(request, str(required_roles), request.user_roles)
                        return JsonResponse(
                    ...
                    middleware_logger.info(f"Acceso permitido para {user_info} en {request.path}")
...
class AuditMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        should_audit = (
            ...
            any(request.path.startswith(path) for path in self.AUDIT_PATHS)
        )
    ...
    def process_response(self, request, response):
        ...
        if hasattr(request, '_perf_start'):
            ...
            if PerformanceMonitoringMiddleware._request_count % 100 == 0:
                middleware_logger.info(f"PERFORMANCE: Processed {PerformanceMonitoringMiddleware._request_count} requests")
