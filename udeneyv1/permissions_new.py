...
class IsOwnerOrReadOnly(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if (request.user.is_superuser or 
            request.user.groups.filter(name='Admin_Negocio').exists()):
            return True
...
class TransaccionPermissions(RoleBasedPermission):
    ...
    def has_permission(self, request, view):
        ...
        if request.method == 'POST':
            return (request.user.is_superuser or 
                    request.user.groups.filter(name='Comprador').exists())

        # Admins y monitores pueden ver todas las transacciones
        if (request.user.is_superuser or
            request.user.groups.filter(name__in=['Admin_Negocio', 'Monitor']).exists()):
            return True
...
class MonitorPermissions(RoleBasedPermission):
    ...
    def has_permission(self, request, view):
        ...
        return (request.user.is_superuser or 
                request.user.groups.filter(name__in=['Monitor', 'Admin_Negocio']).exists())
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not any(group in user_groups for group in group_names):
                return HttpResponseForbidden(f"Acceso denegado: Se requiere uno de estos roles: {', '.join(group_names)}")
